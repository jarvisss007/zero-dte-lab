...
R17_WINDOWS = ["09:45", "10:00", "10:45", "11:20", "13:45", "15:20"]
...
def main() -> None:
    ...
    # ---- pre-registered group test: the six r17 windows -----------------
    r17 = df[df["bucket"].isin(R17_WINDOWS)]
    rest = df[~df["bucket"].isin(R17_WINDOWS)]
    k, n = int(r17["big"].sum()), len(r17)
    p_group = stats.binomtest(k, n, base).pvalue
    print(f"\nr17 window group: {n} obs, big-move freq "
          f"{k/n:.3f} vs baseline {base:.3f} (p={p_group:.4f})")
    print(f"r17 window mean |ret|: {r17['ret'].abs().mean()*1e4:.2f} bp "
          f"vs rest {rest['ret'].abs().mean()*1e4:.2f} bp "
          f"(Mann-Whitney p={stats.mannwhitneyu(r17['ret'].abs(), rest['ret'].abs()).pvalue:.4f})")
    ...
    x = np.arange(len(per))
    colors = ["#c0392b" if b in R17_WINDOWS else ("#2980b9" if s else "#95a5a6")
    ...
    axes[0].set_title("SPY realized vol by 5-min bucket (bp per 5 min) — "
                      "red = r17 windows, blue = significant big-move buckets")
    ...
    for ax in axes:
        ax.set_xticks(x[::6])
        ax.set_xticklabels(per.index[::6], rotation=45)
    fig.tight_layout()
    ...
    ax2.set_title("Mean |5-min return| (bp), day of week x hour ET")
    fig2.colorbar(im)
    fig2.tight_layout()
